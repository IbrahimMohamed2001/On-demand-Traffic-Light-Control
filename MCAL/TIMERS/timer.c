...
void delay_start(uint8_t timer, uint16_t delay_ms, uint16_t prescaler) {
	...
	switch (prescaler) {
		...
		default: prescaler = NO_PRESCALER; break; // wrong prescaler --> No prescaler
	}
	...
	switch (timer) {
		...
		case TIMER1:
			...
			break;
		default: return; /* ERROR invalid input (timer) */ 
	}
	...
	switch (timer) {
		case TIMER0:
			switch (prescaler) {
				...
				default: break; // wrong prescaler --> No prescaler
			}
